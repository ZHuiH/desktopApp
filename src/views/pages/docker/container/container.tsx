...
class DockerContainer extends React.Component<containerInfo, containerState>{
    ...
    private restart(id: string) {
        ...
        window.runCommand('docker', 'restart', id).then(res => {
            this.props.reload()
            this.state.restart.delete(id)
            this.setState({ restart: this.state.restart })
        })
    }
    ...
    private stop(id: string) {
        ...
        window.runCommand('docker', 'stop', id).then(res => {
            this.props.reload()
            this.state.stop.delete(id)
            this.setState({ stop: this.state.stop })
        })
    }
    ...
    private options(record: any) {
        ...
        let title = "停止容器"
        if (this.state.restart.has(id)) {
            title = "重启中"
            action = <Spin />;
        } else if (!record['STATUS'].includes('Up')) {
            title = "重启容器"
